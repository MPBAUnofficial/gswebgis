...
class AmbitoSpaziale(models.Model):
	...
	codice = models.CharField(max_length=20,help_text="Codice utilizzato per identificare univocamente l'ambito",unique='TRUE')
	shape = models.CharField(max_length=50,help_text="Nome dello shapefile da utilizzare per questo ambito")
	grafico = models.CharField(max_length=100,help_text="Il tipo di grafico da utilizzare per questo ambito",choices=FORMATO_GRAFICO)
	formato_layer = models.CharField(max_length=20,help_text="Formato del layer di riferimento utilizzato per per questo ambito",choices=FORMATO_CODICE)
	formato_data = models.CharField(max_length=10, blank="True", null="True",help_text="In caso di tipo vettoriale deve essere di tipo 'd'")
...
class AmbitoVariabile(models.Model):
	label = models.CharField(max_length=50)
	codice = models.CharField(max_length=50,help_text="Codice utilizzato per identificare univocamente l'ambito",unique='TRUE')
...
class CatalogoDati(models.Model):
	...
	def _get_full_codice(self):
		return '%s_%s_%s_%s' % (self.ambito_temporale.codice,self.ambito_climatico.codice,self.ambito_spaziale.codice,self.ambito_variabile.codice)
	full_codice = property(_get_full_codice)
	
	def _get_full_label(self):
		return '%s su %s per %s' % (self.ambito_variabile.label,self.ambito_temporale.label,self.ambito_spaziale.label)
	full_label = property(_get_full_label)
	
	def _get_full_style(self):
		if self.ambito_spaziale.formato_layer == "v":
			style = create_xml(self.full_codice,"~~DATA~~",self.ambito_variabile.colour.split("|"),self.ambito_variabile.values.split("|"))
		else:
	...
	def _get_ambito_variabile_icon(self):
		if self.ambito_variabile.icon != "":
			...
			icon = '%s?HEIGHT=72&WIDTH=72&LAYERS=%s:%s&STYLES=ammcom&SRS=EPSG:32632&FORMAT=image/png&SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&BBOX=612484.75,5059669.0575215,728617.312,5156975.4424785' % (self.geoserver_url,self.layer_workspace,self.full_codice)
		else:
			icon = '%s?HEIGHT=72&WIDTH=72&LAYERS=%s:%s&STYLES=%s&SRS=EPSG:32632&FORMAT=image/png&SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&BBOX=612484.75,5059669.0575215,728617.312,5156975.4424785' % (self.geoserver_url,self.layer_workspace,self.full_codice,self.full_style)
		return icon
	...
	def __unicode__(self):
		return  '%s_%s_%s_%s' % (self.ambito_temporale,self.ambito_climatico,self.ambito_spaziale,self.ambito_variabile)       
